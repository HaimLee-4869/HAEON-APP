...
export function useMonitoringSubjects() {
  const people = useHaenyeo(); const devices = useDevices(); const locations = useLatestLocations(); const risks = useRiskScores();
  ...
  return { data, isLoading: people.isLoading || devices.isLoading || locations.isLoading || risks.isLoading, isError: people.isError || devices.isError || locations.isError || risks.isError };
}
