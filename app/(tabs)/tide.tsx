...
export default function TideScreen() {
  ...
  return <Screen><PageTitle>물때 검색</PageTitle><SearchBar placeholder="원하는 물때 지역을 검색해 주세요" value={query} onChangeText={setQuery} /><View style={styles.list}>{visible.map(([id, region], index) => <Pressable key={id} onPress={() => router.push({ pathname: '/tide/[region]', params: { region: id } })} style={[styles.row, index === visible.length - 1 && styles.last]}><Text style={styles.name}>{region.name}</Text><ChevronRight size={16} color="#A0A6B2" /></Pressable>)}</View></Screen>;
}
const styles = StyleSheet.create({ list: { marginTop: 14, backgroundColor: colors.surface, borderRadius: 14, borderWidth: 1, borderColor: colors.line, overflow: 'hidden' }, row: { minHeight: 58, paddingHorizontal: 14, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', borderBottomWidth: 1, borderBottomColor: '#EEF0F4' }, last: { borderBottomWidth: 0 }, name: { fontSize: 13, fontWeight: '800', color: colors.text } });
